public class Tuba.Views.Notifications : Views.Timeline, AccountHolder, Streamable {
	...
	construct {
		enabled_group_notifications = accounts.active.tuba_api_versions.mastodon >= 2 || InstanceAccount.InstanceFeatures.GROUP_NOTIFCATIONS in accounts.active.tuba_instance_features;
	...
	public override void on_account_changed (InstanceAccount? acc) {
		enabled_group_notifications = acc.tuba_api_versions.mastodon >= 2 || InstanceAccount.InstanceFeatures.GROUP_NOTIFCATIONS in acc.tuba_instance_features;
		accepts = enabled_group_notifications ? typeof (API.GroupedNotificationsResults.NotificationGroup) : typeof (API.Notification);
		filters_changed (false);
